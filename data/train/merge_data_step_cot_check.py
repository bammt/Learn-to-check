...
for i in range(len(data)):
    ...
    if 'correction' in element.keys():
        ...
        for j in range(element['wrong_step']):
            ...
            original_check=element['check'][j].strip().split('\n')
            
            new_line['answer']=''
            new_line['answer']+=original_check[0].strip()+'\n'
            for k in range(3):
                new_line['answer']+=original_check[k+1][4:]+'\n'
            
            new_line['answer']+=original_check[-1]
        ...
        #check_str='Step {} is wrong and need to be corrected.'.format(element['wrong_step'])
        check_str=list(filter(None, element['check'][element['wrong_step']-1].split('\n')))[-1]
